...
* Constructor
*
* @param xdmData a map representing the initialization data for this {@code IdentityProperties}
*     instance
*/
...
void setECIDSecondary(final ECID newSecondaryEcid) {
	...
	if (getECID() == null) {
	MobileCore.log(
		LoggingMode.DEBUG, LOG_TAG, "Cannot set secondary ECID value as no primary ECID exists.");
	return;
	}
	...
	final IdentityItem newSecondaryECIDItem =
		new IdentityItem(newSecondaryEcid.toString(), AuthenticatedState.AMBIGUOUS, false);
...
* identifiers.
*
* <p>Any identifier in the passed in {@code IdentityMap} which has the same id in the same
* namespace will update the current identifier. Any new identifier in the passed in {@code
* IdentityMap} will be added to the current identifiers Certain namespaces are not allowed to be
* modified and if exist in the given customer identifiers will be removed before the update
* operation is executed. The namespaces which cannot be modified through this function call
* include: - ECID - IDFA - GAID
*
...
* identifiers.
*
* <p>Identifiers with following namespaces are prohibited from removing using the API - ECID -
* IDFA - GAID
*
...
*
* @param allowEmpty If this {@link IdentityProperties} contains no data, return a dictionary with
*     a single {@link IdentityMap} key
...
/**
* Filter out any items contained in reserved namespaces from the given {@link IdentityMap}. The
* list of reserved namespaces can be found at {@link #reservedNamespaces}.
*
* @param identityMap the {@code IdentityMap} to filter out items contained in reserved
*     namespaces.
*/
