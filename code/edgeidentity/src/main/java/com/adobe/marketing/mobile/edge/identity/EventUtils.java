...
final class EventUtils {
    ...
    /**
     * Checks if the provided {@code event} is of type {@link
     * IdentityConstants.EventType#EDGE_IDENTITY} and source {@link
    ...
    static boolean isRemoveIdentityEvent(final Event event) {
        return event != null
                ...
                && IdentityConstants.EventSource.REMOVE_IDENTITY.equalsIgnoreCase(
                        event.getSource());
    }
    ...
    static boolean isUpdateIdentityEvent(final Event event) {
        return event != null
                && IdentityConstants.EventType.EDGE_IDENTITY.equalsIgnoreCase(event.getType())
                && IdentityConstants.EventSource.UPDATE_IDENTITY.equalsIgnoreCase(
                        event.getSource());
    }
    ...
    static boolean isRequestIdentityEvent(final Event event) {
        return event != null
                && IdentityConstants.EventType.EDGE_IDENTITY.equalsIgnoreCase(event.getType())
                && IdentityConstants.EventSource.REQUEST_IDENTITY.equalsIgnoreCase(
                        event.getSource());
    }

    /**
     * Checks if the provided {@code event} is of type {@link
     * IdentityConstants.EventType#GENERIC_IDENTITY} and source {@link
    ...
    /**
     * Checks if the provided {@code event} is a shared state update event for {@code
     * stateOwnerName}
     *
     * @param stateOwnerName the shared state owner name; should not be null
     * @param event current event to check; should not be null
     * @return {@code boolean} indicating if it is the shared state update for the provided {@code
     *     stateOwnerName}
     */
    static boolean isSharedStateUpdateFor(final String stateOwnerName, final Event event) {
        ...
        try {
            stateOwner =
                    (String) event.getEventData().get(IdentityConstants.SharedState.STATE_OWNER);
    ...
    /**
     * Extracts the ECID from the Identity Direct shared state and returns it as an {@link ECID}
     * object
     *
