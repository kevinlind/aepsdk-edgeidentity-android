...
/**
* Completes init for the Identity extension. Attempts to load the already persisted identities
* from persistence into {@link #identityProperties} If no ECID is loaded from persistence
* (ideally meaning first launch), then we attempt to read ECID for the direct Identity Extension.
* If there is no ECID loaded from the persistence of direct Identity Extension, then and new ECID
* is generated and persisted finishing the bootUp sequence.
*/
void bootUp() {
	...
	MobileCore.log(
		LoggingMode.DEBUG, LOG_TAG, "IdentityState - Edge Identity has successfully booted up");
}
...
/**
* Update the legacy ECID property with {@code legacyEcid} provided it does not equal the primary
* or secondary ECIDs currently in {@code IdentityProperties}.
*
