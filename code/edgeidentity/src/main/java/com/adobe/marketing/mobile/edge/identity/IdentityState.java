...
class IdentityState {
    ...
    /**
     * Completes init for this Identity extension. Attempts to load the already persisted identities
     * from persistence into {@link #identityProperties} If no ECID is loaded from persistence
     * (ideally meaning first launch), attempts to migrate existing ECID from the direct Identity
     * Extension, either from its persisted store or from its shared state if the direct Identity
     * extension is registered. If no ECID is found for migration, then a new ECID is generated.
     * Stores the {@code identityProperties} once an ECID is set and creates the first shared state.
     *
     * @param callback {@link SharedStateCallback} used to get the EventHub and/or Identity direct
     *     shared state and create a shared state on the EventHub; should not be null
    ...
    boolean bootupIfReady(final SharedStateCallback callback) {
        ...
        if (identityProperties.getECID() == null) {
            ...
            // If direct Identity has no persisted ECID, check if direct Identity is registered with
            // the SDK
            else if (isIdentityDirectRegistered(callback)) {
                final Map<String, Object> identityDirectSharedState =
                        callback.getSharedState(
                ...
                // If there is no direct Identity shared state, abort boot-up and try again when
                // direct Identity shares its state
                else {
                    MobileCore.log(
                            LoggingMode.DEBUG,
                            LOG_TAG,
                            "IdentityState - On bootup direct Identity extension is registered, waiting for its state change.");
                    return false; // If no ECID to migrate but Identity direct is registered, wait
                    // for Identity direct shared state
                }
            }
            // Generate a new ECID as the direct Identity extension is not registered with the SDK
            // and there was no direct Identity persisted ECID
            else {
    ...
    /**
     * Clears all identities and regenerates a new ECID value, then saves the new identities to
     * persistence.
     */
    ...
    /**
     * Update the customer identifiers by merging the passed in {@link IdentityMap} with the current
     * identifiers present in {@link #identityProperties}.
     *
     * @param map the {@code IdentityMap} containing customer identifiers to add or update with the
    ...
    /**
     * Remove customer identifiers specified in passed in {@link IdentityMap} from the current
     * identifiers present in {@link #identityProperties}.
     *
    ...
    /**
     * Update the legacy ECID property with {@code legacyEcid} provided it does not equal the
     * primary or secondary ECIDs currently in {@code IdentityProperties}.
     *
    ...
    /**
     * This method is called when the primary Edge ECID is null and the Identity Direct shared state
     * has been updated (install scenario when Identity Direct is registered). Sets the {@code
     * legacyEcid} as primary ECID when not null, otherwise generates a new ECID.
     *
     * @param legacyEcid the current ECID from the direct Identity extension
     */
    ...
    /**
     * Check if the Identity direct extension is registered by checking the EventHub's shared state
     * list of registered extensions.
     *
     * @param callback the {@link SharedStateCallback} to be used for fetching the EventHub Shared
    ...
    private boolean isIdentityDirectRegistered(final SharedStateCallback callback) {
        Map<String, Object> registeredExtensionsWithHub =
                callback.getSharedState(IdentityConstants.SharedState.Hub.NAME, null);
            ...
            try {
                final Map<String, Object> extensions =
                        (HashMap<String, Object>)
                                registeredExtensionsWithHub.get(
