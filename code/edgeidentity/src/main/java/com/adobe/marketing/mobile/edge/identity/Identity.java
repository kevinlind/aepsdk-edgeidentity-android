...
public class Identity {
	...
	*     If an {@link AdobeCallbackWithError} is provided, an {@link AdobeError} can be returned
	*     in the eventuality of any error that occurred while getting the Experience Cloud ID
	*/
				...
				new AdobeCallback<Event>() {
					...
					public void call(Event responseEvent) {
						...
						final IdentityMap identityMap =
								IdentityMap.fromXDMMap(responseEvent.getEventData());
						...
						final List<IdentityItem> ecidItems =
								identityMap.getIdentityItemsForNamespace(
										IdentityConstants.Namespaces.ECID);

						if (ecidItems == null
								|| ecidItems.isEmpty()
								|| ecidItems.get(0).getId() == null) {
							callback.call("");
						} else {
	...
	*/
	...
	/**
	* Removes the identity from the stored client-side {@link IdentityMap} and XDM shared state.
	* The Identity extension will stop sending this identifier. This does not clear the identifier
	* from the User Profile Graph. Identifiers which have an empty `id` or empty `namespace` are
	* not allowed and are ignored.
	*
	...
	public static void removeIdentity(final IdentityItem item, final String namespace) {
		...
		if (item == null) {
			MobileCore.log(
					LoggingMode.DEBUG,
					LOG_TAG,
					"Identity - Unable to removeIdentity, IdentityItem is null");
			return;
		}
	...
	*
	* @param callback {@link AdobeCallback} invoked with the current {@link IdentityMap} If an
