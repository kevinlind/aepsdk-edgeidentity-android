...
/**
* Returns the Experience Cloud ID. An empty string is returned if the Experience Cloud ID was
* previously cleared.
*
* @param callback {@link AdobeCallback} of {@link String} invoked with the Experience Cloud ID If
*     an {@link AdobeCallbackWithError} is provided, an {@link AdobeError} can be returned in the
...
/**
* Updates the currently known {@link IdentityMap} within the SDK and XDM shared state. The
* Identity extension will merge the received identifiers with the previously saved one in an
* additive manner, no identifiers will be removed using this API.
*
		...
		public void error(final ExtensionError extensionError) {
			MobileCore.log(
				...
				String.format(
					...
					IdentityConstants.EventNames.UPDATE_IDENTITIES, extensionError.getErrorName()));
		}
		};

	final Event updateIdentitiesEvent =
		new Event.Builder(
...
/**
* Removes the identity from the stored client-side {@link IdentityMap} and XDM shared state. The
* Identity extension will stop sending this identifier. This does not clear the identifier from
* the User Profile Graph. Identifiers which have an empty `id` or empty `namespace` are not
* allowed and are ignored.
*
...
public static void removeIdentity(final IdentityItem item, final String namespace) {
	...
	if (item == null) {
	MobileCore.log(
		LoggingMode.DEBUG, LOG_TAG, "Identity - Unable to removeIdentity, IdentityItem is null");
	return;
	}
		...
		public void error(final ExtensionError extensionError) {
			MobileCore.log(
				...
				String.format(
					...
					IdentityConstants.EventNames.REMOVE_IDENTITIES, extensionError.getErrorName()));
		}
		};

	final Event removeIdentitiesEvent =
		new Event.Builder(
...
*
* @param callback {@link AdobeCallback} invoked with the current {@link IdentityMap} If an {@link
...
private static <T> void returnError(final AdobeCallback<T> callback, final AdobeError error) {
	...
	final AdobeCallbackWithError<T> adobeCallbackWithError =
		callback instanceof AdobeCallbackWithError ? (AdobeCallbackWithError<T>) callback : null;
