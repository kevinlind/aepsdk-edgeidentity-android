...
interface SharedStateCallback {
    ...
    /**
     * Fetches the Shared State for the provided {@code event} from the specified {@code
     * stateOwner}.
     *
     * @param stateOwner Shared state owner name
     * @param event current event for which to fetch the shared state; if null is passed, the latest
     *     shared state will be returned
     ...
     * @param event current event for which to set the shared state; if null is passed, the next
     *     shared state version will be set
     */
