...
/**
* Converts provided {@link JSONArray} into {@link List} for any number of levels which can be
* used as event data This method is recursive. The elements for which the conversion fails will
* be skipped.
*
* @param jsonArray to be converted
* @return {@link List} containing the elements from the provided json, null if {@code jsonArray}
*     is null
*/
...
/**
* Serialize the given {@code jsonString} to a JSON Object, then flattens to {@code Map<String,
* String>}. If the provided string is not in JSON structure an {@link JSONException} is thrown.
*
...
* For example, a JSON such as "{xdm: {stitchId: myID, eventType: myType}}" is flattened to two
* map elements "xdm.stitchId" = "myID" and "xdm.eventType" = "myType".
*
...
*
* <p>Method is called recursively. To use, call with an empty path such as {@code addKeys("", new
* ObjectMapper().readTree(JsonNodeAsString), map);}
*
...
/**
* Class similar to {@link IdentityItem} for a specific namespace used for easier testing. For
* simplicity this class does not involve authenticatedState and primary key
*/
