...
public void setup() {
	...
	Mockito.when(mockContext.getSharedPreferences(IdentityConstants.DataStoreKey.DATASTORE_NAME, 0))
...
public void test_ListenersRegistration() {
	...
	verify(mockExtensionApi, times(7))
		.registerEventListener(
			anyString(), anyString(), any(Class.class), any(ExtensionErrorCallback.class));
...
public void test_handleUpdateIdentities() throws Exception {
	// setup
	Map<String, Object> identityXDM = createXDMIdentityMap(new TestItem("UserId", "secretID"));
	final ArgumentCaptor<Map> sharedStateCaptor = ArgumentCaptor.forClass(Map.class);
...
public void test_handleUpdateIdentities_DoNotUpdateReservedNamespace() throws Exception {
	...
	Map<String, String> sharedState = flattenMap(sharedStateCaptor.getValue());
	assertEquals(
		6, sharedState.size()); // 6 represents id, primary and authState of USERID identifier and
	// generated ECID
	...
	Map<String, String> persistedData =
		flattenJSONString(persistenceValueCaptor.getAllValues().get(1));
	assertEquals(
		6, persistedData.size()); // 3 represents id, primary and authState of USERID identifier and
	// generated ECID
	assertEquals("somevalue", persistedData.get("identityMap.UserId[0].id"));
	assertNotEquals(
		"somevalue",
		persistedData.get("identityMap.ECID[0].id")); // verify that the ECID is not disturbed
}
...
public void test_handleUpdateIdentities_CaseSensitiveNamespace_OnCustomIdentifiers()
	...
	Map<String, Object> identityXDM =
		createXDMIdentityMap(
			new TestItem("pushToken", "somevalue"), new TestItem("PUSHTOKEN", "SOMEVALUE"));
	Event updateIdentityEvent = buildUpdateIdentityRequest(identityXDM);
	extension.handleUpdateIdentities(updateIdentityEvent);
...
public void test_handleRemoveIdentity() throws Exception {
	// setup
	Map<String, Object> identityXDM =
		createXDMIdentityMap(new TestItem("UserId", "secretID"), new TestItem("PushId", "token"));
