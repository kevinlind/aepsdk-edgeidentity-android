...
/**
* Waits for all the {@code #knownThreads} to finish or fails the test after timeoutMillis if some
* of them are still running when the timer expires. If timeoutMillis is 0, a default timeout will
* be set = 1000ms
*
...
/**
* Checks if current thread is not a daemon and its name starts with one of the known thread names
* specified here {@link #knownThreads}
*
...
/**
* Sets an expectation for a specific event type and source and how many times the event should be
* dispatched.
*
...
*
* @param ignoreUnexpectedEvents if set on false, an assertion is made on unexpected events,
*     otherwise the unexpected events are ignored
...
/**
* Asserts if any unexpected event was received. Use this method to verify the received events are
* correct when setting event expectations. Waits a short time before evaluating received events
* to allow all events to come in.
*
...
/**
* Asserts if any unexpected event was received. Use this method to verify the received events are
* correct when setting event expectations.
*
...
/**
* Returns the {@code Event}(s) dispatched through the Event Hub, or empty if none was found. Use
* this API after calling {@link #setExpectationEvent(String, String, int)} to wait for the
* expected events. The wait time for each event is {@link #WAIT_EVENT_TIMEOUT_MS}ms.
*
* @param type the event type as in the expectation
* @param source the event source as in the expectation
* @return list of events with the provided {@code type} and {@code source}, or empty if none was
*     dispatched
* @throws InterruptedException
...
/**
* Returns the {@code Event}(s) dispatched through the Event Hub, or empty if none was found. Use
* this API after calling {@link #setExpectationEvent(String, String, int)} to wait for the right
* amount of time
*
* @param type the event type as in the expectation
* @param source the event source as in the expectation
...
/**
* Synchronous call to get the shared state for the specified {@code stateOwner}. This API throws
* an assertion failure in case of timeout.
*
