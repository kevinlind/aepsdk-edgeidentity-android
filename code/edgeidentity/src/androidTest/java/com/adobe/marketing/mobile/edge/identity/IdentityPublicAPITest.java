...
public void testRegisterExtensionAPI() throws InterruptedException {
	...
	Map<String, String> sharedStateMap =
		flattenMap(getSharedStateFor(IdentityTestConstants.SharedStateName.EVENT_HUB, 1000));
	assertEquals(
...
public void testUpdateAPI_nullData() throws Exception {
	...
	List<Event> dispatchedEvents =
		getDispatchedEventsWith(
			IdentityConstants.EventType.HUB, IdentityConstants.EventSource.SHARED_STATE);
...
public void testUpdateAPI_withReservedNamespaces() throws Exception {
	...
	assertNotEquals(
		"newECID", persistedMap.get("identityMap.ECID[0].id")); // ECID doesn't get replaced by API
}
...
public void testGetIdentities() {
	...
	IdentityMap responseMap =
		(IdentityMap) getIdentitiesResponse.get(IdentityTestConstants.GetIdentitiesHelper.VALUE);
