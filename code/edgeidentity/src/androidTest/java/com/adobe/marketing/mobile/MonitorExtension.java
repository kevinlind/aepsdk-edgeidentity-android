	...
	MobileCore.registerExtension(
		...
		public void error(ExtensionError extensionError) {
			MobileCore.log(
				LoggingMode.ERROR,
				LOG_TAG,
				"There was an error registering the Monitor extension: "
					+ extensionError.getErrorName());
		}
		});
}
...
/**
* Processor for all heard events. If the event type is of this Monitor Extension, then the action
* is performed per the event source. All other events are added to the map of received events. If
* the event is in the map of expected events, its latch is counted down.
*
* @param event
*/
public void wildcardProcessor(final Event event) {
	...
	} else if (TestConstants.EventSource.XDM_SHARED_STATE_REQUEST.equalsIgnoreCase(
		event.getSource())) {
...
/**
* Processor which retrieves and dispatches the XDM shared state for the state owner specified in
* the request.
*
* @param event
*/
...
/**
* Processor which retrieves and dispatches the shared state for the state owner specified in the
* request.
*
* @param event
*/
